...
Apple Music TTML may split words into multiple `<span>` elements for syllable-level timing (e.g. `<span>Ri</span><span>sin'</span>`). The parser treats each `<span>` as a separate `LyricWord` — it does NOT attempt to merge syllables, since Apple Music’s format provides no machine-readable word boundary signal. Instead, syllable handling is the chord matcher’s responsibility: both `mapChordsToCharPositions` and `similarity` try concatenating 2–3 consecutive TTML words to match a single UG word (e.g. TTML "Ri"+"sin'" → UG "Risin'"). Word alignment also uses Levenshtein-based fuzzy matching (≥75% ratio, min 3 chars) so e.g. "risin'" ≈ "rising". This keeps the parser simple and the matching robust without linguistic heuristics.

## Word-level highlighting
`LyricLine` renders word `<span>`s with `word-past` / `word-active` / `word-upcoming` classes based on `currentTimeMs` against each `LyricWord.beginMs/endMs`. For syllable-split words this means each syllable lights up individually, which is more precise than whole-word highlighting. The effect is only visible on the active line (scoped via `.line.active .word-*` CSS). Detection uses `hasWordTiming` — a line is considered to have word timing when at least one word's timing diverges from the line's own `beginMs/endMs` (skips the synthetic fallback words the parser emits for chord lines lacking per-word spans).
