...
The serializer emits syllable spans adjacently with space text nodes between words, matching the raw Apple Music format for correct roundtripping.

## Word-level highlighting
`LyricLine` renders word `<span>`s with `word-past` / `word-active` / `word-upcoming` classes based on `currentTimeMs` against each `LyricWord.beginMs/endMs`. When a word has `syllables`, each syllable is rendered as a separate `<span>` with its own timing class for sub-word highlighting precision. The effect is only visible on the active line (scoped via `.line.active .word-*` CSS). Detection uses `hasWordTiming` — a line is considered to have word timing when at least one word's timing diverges from the line's own `beginMs/endMs` (skips the synthetic fallback words the parser emits for chord lines lacking per-word spans). `chordTimeMs` in `LyricsDisplay.vue` also uses syllable boundaries when available for more precise chord activation timing.
